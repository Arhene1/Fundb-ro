...
if page == "📸 Fundstück melden":
    ...
    uploaded_photos = st.file_uploader(
        "📸 Bilder der Fundsache hochladen",
        type=[
            "jpg",
            "jpeg",
            "png",
        ...
        help=(
            "Du kannst mehrere Bilder auswählen, z. B. Vorderseite, "
            "Rückseite und besondere Merkmale. Auf dem Smartphone "
            "kannst du je nach Browser auch direkt die Kamera verwenden."
        ),
    )
    ...
    if uploaded_photos:
        ...
        st.markdown("### 🖼️ Hochgeladene Bilder")

        image_columns = st.columns(min(len(uploaded_photos), 3))
        ...
        for index, uploaded_photo in enumerate(uploaded_photos):
            ...
            with image_columns[index % len(image_columns)]:
                st.image(
                    image,
                    caption=f"Bild {index + 1}",
                    use_container_width=True,
                )

        # Für die KI wird weiterhin das erste Bild verwendet.
    ...
    if st.button(
        ...
        if not title.strip():
            ...
            # Das erste Bild bleibt zusätzlich in "foto" gespeichert,
            # damit ältere Datensätze und bestehende Funktionen kompatibel bleiben.
            if saved_photos:
                saved_photo = Path(saved_photos[0])
    ...
    if not items:
        ...
        for item in items:
            ...
            with st.container(
                ...
                if item["fotos"]:
                    try:
                        photo_paths = json.loads(item["fotos"])
                    except (json.JSONDecodeError, TypeError):
                        photo_paths = []

                # Alte Datensätze mit nur einem Foto bleiben kompatibel.
                if not photo_paths and item["foto"]:
                    photo_paths = [item["foto"]]

                if photo_paths:
                    st.markdown("**🖼️ Bilder der Fundsache**")
                    ...
                    if valid_photo_paths:
                        image_columns = st.columns(
                            min(len(valid_photo_paths), 3)
                        )

                        for index, photo_path in enumerate(valid_photo_paths):
                            with image_columns[index % len(image_columns)]:
                                st.image(
                                    photo_path,
                                    caption=f"Bild {index + 1}",
